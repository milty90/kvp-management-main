...
export default function ButtonGroup({ filter }: ButtonGroupProps) {
  ...
  return (
    <div
      className={`${width < 768 ? "absolute -top-5 z-10 w-17 items-start gap-1 bg-gray-200/90" : "inline-flex"} items-center gap-1 bg-gray-200/80 p-1 rounded-xl`}
    >
      {visibleTabs.map((tab) => (
        <button
          key={tab}
          ...
          className={`
             ${width < 768 ? "min-w-15 p-1.5 rounded-lg text-sm font-medium transition-all duration-150" : " px-3 md:px-4 py-1.5  rounded-lg text-sm font-medium transition-all duration-150"}
              ${
                selected === tab
