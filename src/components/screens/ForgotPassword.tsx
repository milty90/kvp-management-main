    ...
    <div className="flex flex-col items-center md:justify-center  h-screen bg-background">
      <div className="flex flex-col pt-12 items-center justify-start h-screen md:h-auto md:justify-center w-full max-w-md shadow-md md:rounded-2xl bg-surface p-8">
        ...
        <form className=" w-full mb-1 max-w-sm" onSubmit={handlePasswordReset}>
          <div className="mb-6 items-center justify-items-center">
            ...
            <input
              type="email"
              id="email"
              className={`w-full px-3 py-2 border ${theme === "dark" ? "border-gray-600 focus:ring-green-600" : "border-gray-300 focus:ring-blue-500"} rounded-lg focus:outline-none focus:ring-2 `}
              value={email}
        ...
        <button
          ...
          className={
            "inline-block align-baseline font-semibold text-sm text-button hover:text-button-hover cursor-pointer mt-4"
          }
        >
          {translation.forgotPasswordScreen.backToLogin}
        </button>
      </div>
    </div>
  );
